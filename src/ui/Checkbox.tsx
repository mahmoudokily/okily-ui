...
export const RadioLabel = styled.label.attrs<RadioLabelProps>(({ withSpace, mr, my }) => ({
  mr: withSpace ? 3 : mr,
  my: withSpace ? 2 : my
})) <RadioLabelProps>`
    display: inline-flex;
...
export const RadioSpan = styled.span(({ theme }) => `
    flex: 1 1 auto;
...
${(({ theme, variant = 'primary', $fill, shape, disabled, $size }) => `
    ...
    + ${Rectangle} {
        ...
        background: ${$fill ? theme.type[variant]?.form?.radioBorder : theme.type[variant]?.form?.inputBackground};
...
export const Checkbox = React.forwardRef<Props, any>(({
  ...
  return (
    <RadioLabel withSpace={children ? true : false} {...containerProps}>
      <Input ref={ref} type='checkbox' shape={shape} $fill={fill} variant={variant} {...inputProps}{...props} />
      <Rectangle>
        <StickContainer>
          <ShortStick />
          <LongStick />
