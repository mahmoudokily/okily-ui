...
import { Variant, Size, Shape } from "../helpers/types";
...
export const PopoverElement = styled(Box) <Props>`
  ${({ theme, variant = "danger", arrow, isArrowInHeader = true }) => `
   position: fixed;
...
?      }
      .src-popover-header {
            border-bottom: solid 1px ${theme.type[variant]?.popover?.headerBorder
    };
      ...
      ? `
        ${ArrowElement} {
                &:before {
                    ...
                    border-bottom-color: ${theme.type[variant]?.popover?.border
      };
                }
                &:after {
                    top: 1px;
                    border-width: 0 .5rem .5rem .5rem;
                    border-bottom-color: ${theme.type[variant]?.popover?.background
      };
                }
            }
        }
