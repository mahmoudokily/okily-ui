...
export default class Popover extends React.Component<Props, State> {
  ...
  render() {
    ...
    const Element = (
      <PopoverElement maxWidth="300px" {...p.elementProps} style={{ ...(p?.elementProps?.style || DEFAULT_STYLE), transform: s.attrs.transform }} ref={this.el} size$={p.size} size$Sm={p.sizeSm} size$Md={p.sizeMd} size$Lg={p.sizeLg} size$Xl={p.sizeXl} variant={p.variant} arrow={s.attrs.arrow} isArrowInHeader={s.attrs.isArrowInHeader}>
        {p.children[1]}
